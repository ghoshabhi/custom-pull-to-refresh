  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.refreshing && !this.state.animating) {
      return this.triggerAnimation(() => {
        // reset the position of the gears when
        // animated sequence has finished
        ...
        <Animated.Image
          style={[
            styles.gearThree,
            {
              transform: [
                {
                  rotate: this.state.animating
                    ? interpolatedGearThreeRotation
                    : this.props.anticlockwiseRotation
                },
